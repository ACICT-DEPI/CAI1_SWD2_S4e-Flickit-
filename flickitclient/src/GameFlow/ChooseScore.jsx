    ...
    return (
        <div className="flex " >
            ...
            <div className=" relative flex-grow flex items-center justify-center bg-gray-200  h-screen " style={{
                ...
                <div className="absolute top-24 sm:ml-16 text-3xl font-bold font-Risque text-stone-50"> <label> Choose the score you want for this game play!</label></div>
                <div className="Scoring-window flex-wrap flex items-center justify-center w-3/5 top-0 mt-24 mb-32 sm:top-16 sm:mb-16">
                {scores.map( score => {
                    return  <button key={score} className={`border-solid border-4 rounded-3xl md:w-40 md:h-40 sm:w-24 sm:h-24 m-5 text-7xl  ${isClicked === score? "bg-gray-100 border-4" : " text-white bg-gradient-to-b from-green-500  to-green-400"}`} onClick={()=>{handleScore(score)}}> {score} </button>
                })}
                    
                </div>
                <div className="absolute bottom-0 flex">
                <img className=" w-24 h-24" src={coin}></img>
                    <button className="border-solid rounded-3xl border-4 w-64 h-20 mt-2 ml-5 mr-5 mb-5 bg-purple-900 text-3xl font-bold  font-Risque text-stone-50"  onClick={() =>{handleSubmit()} }> confirm choice </button>
                    <img className=" w-24 h-24" src={coin}></img>
